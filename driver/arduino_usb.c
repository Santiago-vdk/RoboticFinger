...
//USB_PIC18F_MINOR_BASE
    interface = usb_find_interface(&pic18f_driver, subminor);
    if (!interface)
    {
...
*/
printk("Endpoints: %d", iface_desc->desc.bNumEndpoints);
...
return 0;
	...
	printk(KERN_INFO "ED[1]->bmAttributes: 0x%02X\n", endpoint->bmAttributes);
	printk(KERN_INFO "ED[1]->wMaxPacketSize: 0x%04X (%d)\n", endpoint->wMaxPacketSize,endpoint->wMaxPacketSize);
	...
	dev->bulk_in_buffer = kmalloc(buffer_size, GFP_KERNEL);
        ...
        endpoint = (struct usb_endpoint_descriptor*)&iface_desc->endpoint[1].desc;

	printk(KERN_INFO "ED[0]->bEndpointAddress: 0x%02X\n", endpoint->bEndpointAddress);
	printk(KERN_INFO "ED[0]->bmAttributes: 0x%02X\n", endpoint->bmAttributes);
	printk(KERN_INFO "ED[0]->wMaxPacketSize: 0x%04X (%d)\n", endpoint->wMaxPacketSize,endpoint->wMaxPacketSize);
