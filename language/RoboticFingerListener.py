# Generated from RoboticFinger.g4 by ANTLR 4.7
from antlr4 import *
if __name__ is not None and "." in __name__:
    from .RoboticFingerParser import RoboticFingerParser
else:
    from RoboticFingerParser import RoboticFingerParser

# This class defines a complete listener for a parse tree produced by RoboticFingerParser.
class RoboticFingerListener(ParseTreeListener):

    # Enter a parse tree produced by RoboticFingerParser#chat.
    def enterChat(self, ctx:RoboticFingerParser.ChatContext):
        pass

    # Exit a parse tree produced by RoboticFingerParser#chat.
    def exitChat(self, ctx:RoboticFingerParser.ChatContext):
        pass


    # Enter a parse tree produced by RoboticFingerParser#line.
    def enterLine(self, ctx:RoboticFingerParser.LineContext):
        pass

    # Exit a parse tree produced by RoboticFingerParser#line.
    def exitLine(self, ctx:RoboticFingerParser.LineContext):
        pass


    # Enter a parse tree produced by RoboticFingerParser#command.
    def enterCommand(self, ctx:RoboticFingerParser.CommandContext):
        pass

    # Exit a parse tree produced by RoboticFingerParser#command.
    def exitCommand(self, ctx:RoboticFingerParser.CommandContext):
        pass


    # Enter a parse tree produced by RoboticFingerParser#positionX.
    def enterPositionX(self, ctx:RoboticFingerParser.PositionXContext):
        pass

    # Exit a parse tree produced by RoboticFingerParser#positionX.
    def exitPositionX(self, ctx:RoboticFingerParser.PositionXContext):
        pass


    # Enter a parse tree produced by RoboticFingerParser#positionY.
    def enterPositionY(self, ctx:RoboticFingerParser.PositionYContext):
        pass

    # Exit a parse tree produced by RoboticFingerParser#positionY.
    def exitPositionY(self, ctx:RoboticFingerParser.PositionYContext):
        pass


